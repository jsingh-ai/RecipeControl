...
`rc_segment`, `rc_condition_interval`, and `rc_boundary_event` contain compressed engine output. Segment boundaries have no update endpoint. Segment labels allow Good, Bad, Unsure, or NULL/Unlabeled. Training eligibility is computed only for Good/Bad segments that are neither Data Gap nor Insufficient History.
