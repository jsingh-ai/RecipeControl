...
test('create rule, analyze, label, reload, detect duplicate, and change timezone', async ({ page }) => {
  ...
  await page.route('**/api/**', async (route) => {
    ...
    if (path.includes('/machines/1/tags')) return fulfill([{ key: 'temperature', display_name: 'Temperature', data_type: 'numeric', units: '°F' }, { key: 'pressure', display_name: 'Pressure', data_type: 'numeric', units: 'psi' }, { key: 'speed', display_name: 'Speed', data_type: 'numeric', units: 'ft/min' }, { key: 'alarm_code', display_name: 'Alarm Code', data_type: 'numeric' }, { key: 'motor_running', display_name: 'Motor Running', data_type: 'boolean' }])
  ...
  await page.getByLabel('Variable 1').selectOption('temperature')
  ...
  await page.getByLabel('Variable 2').selectOption('pressure')
  ...
  await page.getByLabel('Variable 1').nth(1).selectOption('alarm_code')
  ...
  await page.getByLabel('Variable 2').nth(1).selectOption('motor_running')
